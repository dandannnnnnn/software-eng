...
1.  **Stap 1 (Registratie):** Een nieuwe gebruiker (User B) registreert zich met een geldig e-mailadres.
    *   **Actie:** `POST /api/user/register` (BB-01).
    *   **Whitebox Check:** Controleer of het wachtwoord gehasht is in de database via Argon2/BCrypt (WB-01).
2.  **Stap 2 (Post Creatie):** User B maakt een nieuwe blogpost aan met de status 'Published'.
    *   **Actie:** `createPost("Ongepaste Titel", "Ongepaste Content")` (TC-P1).
    *   **Whitebox Check:** Verifieer of de `Post` entiteit correct is gekoppeld aan de `User` ID in de database (WT-P4).
...
3.  **Stap 3 (Volgen):** Een bestaande gebruiker (User A) volgt User B.
    *   **Actie:** Klik op 'Volgen' bij User B (BB-06).
    *   **Whitebox Check:** Controleer of de `target_id` niet gelijk is aan `follower_id` (WB-05).
4.  **Stap 4 (Blokkeren):** User A vindt de content van User B aanstootgevend en blokkeert User B.
    *   **Actie:** Klik op 'Blokkeer' bij User B (BB-10).
    *   **Whitebox Check:** Verifieer of de SQL-query voor de feed van User A nu een `LEFT JOIN` bevat op de `blocks` tabel om de content van User B te filteren (WB-08).
